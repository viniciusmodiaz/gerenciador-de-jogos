...
use App\Repositories\UrlParidasDiretasRepository;

class FetchPartidasDiretas extends Command
{
    protected $signature = 'fetch:partidas_diretas';
    ...
    public function handle(TimeRepository $timeRepository, PartidaRepository $partidaRepository, CompeticaoRepository $competicaoRepository, UrlParidasDiretasRepository $ubrlParidasDiretasRepository)
    {
        $paridas = $ubrlParidasDiretasRepository->allUrls();
        foreach ($paridas as $partida) {
            $response = Http::get($partida->url);
            ...
            if ($response->successful()) {
                ...
                $ubrlParidasDiretasRepository->changeSinc($partida->id);
